...
(function(){
 ...
 $('events').innerHTML=
  ...
  +`<div class="card"><h2>2026 Local WordCamps, Campus Connect, and Newer Formats by Country</h2>${cl}
 ...
 /* Community-events-by-year: hover a bar segment to see "<category>: <count>" */
 (function(){
   const wrap=$('byyear'), tip=$('bytip'); if(!wrap||!tip) return;
   wrap.addEventListener('mousemove',e=>{
     const b=e.target.closest?e.target.closest('.bar[data-t]'):null;
     if(!b){tip.style.display='none';return;}
     tip.textContent=b.getAttribute('data-t');
     const r=wrap.getBoundingClientRect(); tip.style.display='block';
     let lx=Math.min(e.clientX-r.left+12, r.width-tip.offsetWidth-6);
     tip.style.left=Math.max(4,lx)+'px'; tip.style.top=Math.max(4,e.clientY-r.top+12)+'px';
   });
